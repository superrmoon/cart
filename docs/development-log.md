...
### 1. 쇼핑 리스트 관리
- 목록 생성 / 수정 / 삭제
- 품목 추가 (이름, 날짜, 금액 — 음수 금액 허용, 차감·할인·환불 용도)
- 품목 완료 체크 (체크박스)
- 완료 품목 숨김/보이기 토글
...
### Phase 6: 음수 금액 지원 (2026-07-21)
- 음수 금액 등록 허용 (차감·할인·환불을 합계에 반영)
- Phase 4의 "금액 음수 방지" 검증을 의도적으로 해제 — 모달 입력·URL API 모두 `Math.trunc`만 적용
- 입력란 `min="0"` 제거, `inputmode="numeric"` 제거 (iOS 숫자 키패드에 마이너스 키가 없어 기본 숫자 키보드로 전환)
